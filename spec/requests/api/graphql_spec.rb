...
describe 'GraphQL' do
  ...
  describe 'testing for Gitaly calls' do
    ...
    let(:query) do
      graphql_query_for('project', { 'fullPath' => project.full_path }, %w(id forksCount))
    end
    ...
    context 'when Gitaly is called' do
      ...
      it "logs a warning that the 'calls_gitaly' field declaration is missing" do
        ...
        expect{post_graphql(query, current_user: user)}.to raise_error(/please add `calls_gitaly: true`/)
      end
    end
  end
end
